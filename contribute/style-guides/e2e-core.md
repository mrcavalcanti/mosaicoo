...
- `./scripts/grafana-server/start-server` This creates a fresh new mosaicoo server working dir, setup's config and starts the server. It will also kill any previously started server that is still running using pid file at _\<repo-root>/scripts/grafana-server/tmp/pid_.
