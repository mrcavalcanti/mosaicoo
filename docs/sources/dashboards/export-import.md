...
Dashboards are exported in Mosaicoo JSON format, and contain everything you need (layout, variables, styles, data sources, queries, etc) to import the dashboard at a later time.
...
In step 2 of the import process Mosaicoo will let you change the name of the dashboard, pick what
...
## Import and sharing with Mosaicoo 2.x or 3.0

Dashboards on Grafana.com use a new feature in Mosaicoo 3.1 that allows the import process
...
Dashboards exported from Mosaicoo 3.1+ have a new json section `__inputs`
...
These inputs and their usage in data source properties are automatically added during export in Mosaicoo 3.1.
If you run an older version of Mosaicoo and want to share a dashboard on Grafana.com you need to manually
add the inputs and templatize the data source properties like above.

If you want to import a dashboard from Grafana.com into an older version of Mosaicoo then you can either import
...
In Mosaicoo v5.3.4+ the export modal has new checkbox for sharing for external use (other instances). If the checkbox is not checked then the `__inputs` section will not be included in the exported JSON file.
