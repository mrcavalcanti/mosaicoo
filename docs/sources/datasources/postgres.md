...
| `SSL Auth Details Method` | Determines whether the SSL Auth details will be configured as a file path or file content. Mosaicoo v7.5+                                                                                                                                |
...
| `Version`                 | Determines which functions are available in the query builder (only available in Mosaicoo 5.3+).                                                                                                                                         |
| `TimescaleDB`             | A time-series database built as a PostgreSQL extension. When enabled, Mosaicoo uses `time_bucket` in the `$__timeGroup` macro to display TimescaleDB specific aggregate functions in the query builder (only available in Mosaicoo 5.3+). |
...
In the FROM field, Mosaicoo will suggest tables that are in the `search_path` of the database user. To select a table or view not in your `search_path`
...
| `$__timeGroup(dateColumn,'5m', 0)`                    | Same as above but with a fill parameter so missing points in that series will be added by mosaicoo and 0 will be used as value.                                                                               |
...
From Mosaicoo 4.3.0 to 4.6.0, template variables are always quoted automatically. If your template variables are strings, do not wrap them in quotes in where clauses.
