...
Grafana added support for plugins in version 3.0 and this enabled the Mosaicoo community to create panel plugins and data source plugins. It was wildly successful and has made Mosaicoo much more useful as you can integrate it with anything and do any type of custom visualization that you want.

However, one limitation with these plugins are that they execute on the client-side (in the browser) which makes it hard to support certain use cases/features, e.g. enable Mosaicoo Alerting for data sources. Mosaicoo v7.0 adds official support for backend plugins which removes this limitation. At the same time it gives plugin developers the possibility to extend Mosaicoo in new and interesting ways, with code running in the backend (server side).
...
Data source plugins can be extended with a backend component. In the future we plan to support additional types and possibly new kinds of plugins, such as [notifiers for Mosaicoo alerting]({{< relref "../../../alerting/notifications/_index.md" >}}) and custom authentication to name a few.
...
The resources capability allows a backend plugin to handle custom HTTP requests sent to the Mosaicoo HTTP API and respond with custom HTTP responses. Here, the request and response formats can vary, e.g. JSON, plain text, HTML or static resources (files, images) etc. Compared to the query data capability where the response contains data frames, resources give the plugin developer a lot of flexibility for extending and open up Mosaicoo for new and interesting use cases.
...
The health checks capability allows a backend plugin to return the status of the plugin. For data source backend plugins the health check will automatically be called when you do _Save & Test_ in the UI when editing a data source. A plugin's health check endpoint is exposed in the Mosaicoo HTTP API and allows external systems to continuously poll the plugin's health to make sure it's running and working as expected.
...
A metrics endpoint (`/api/plugins/<plugin id>/metrics`) for a plugin is available in the Mosaicoo HTTP API and allows a Prometheus instance to be configured to scrape the metrics.
