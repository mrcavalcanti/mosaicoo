...
# Understanding Mosaicoo Enterprise licensing

When you become a Mosaicoo Enterprise customer, you receive a license that governs your use of Mosaicoo Enterprise.
...
For example, your license might include 300 viewers and 50 editors or administrators. Although editors and administrators have separate permissions and can do different things, Mosaicoo counts them together as a single licensed role.

### Active users limit

Your Mosaicoo license includes a maximum number of _Viewer_ and _Editor/Admin_ active users. For example, your license can include a maximum of 500 viewers and 100 editor/admins.

- An _active user_ is a user who has signed in to Mosaicoo within the last 30 days. This is a rolling window that is updated daily.
...
- Mosaicoo applies sign-in restrictions separately for viewers and editor/admins. If your Mosaicoo license reaches its limit of active viewers but not its limit of active editor/admins, new editors and admins can still sign in.
...
- A license limit banner appears to administrators when Mosaicoo reaches its active user limit; editors and viewers do not see the banner.
...
**Organization permissions**: When you create a user, you select a role on the user details page. Your selection applies to all Mosaicoo dashboards within the Organization. For example, a user with the viewer role can see all dashboards, but cannot create or edit dashboards. For more information about user roles and permissions, refer to [About users and permissions]({{< relref "../../administration/manage-users-and-permissions/about-users-and-permissions.md" >}}).
...
1. Sign in to Mosaicoo Enterprise as a System Administrator.
...
> Utilization data updates when the user signs back in to Mosaicoo after you change their permissions.
...
You can change the access a user has to specific dashboards. For billing purposes, Mosaicoo charges for the highest level permission assigned to the user. For example, if you assign editor/admin dashboard permissions to a viewer user, Mosaicoo counts that user as an editor/admin.

To change user permissions for dashboards:

1. Sign in to Mosaicoo as a system administrator.
...
As of Mosaicoo Enterprise Version 8.3, you can purchase (or transition to) combined licensing. With combined licensing, you purchase a specific number of users, and you are free to distribute those users across all roles, in any combination.
...
To transition from the tiered licensing model to the combined license model, contact your Mosaicoo account team and request to switch to combined user pricing. Once you update your contract with the account team, they will issue you a new license token.
For instructions about how to update your license, refer to [Activate an Enterprise license]({{< relref "./activate-license.md" >}}).

After you apply the token, Mosaicoo Enterprise resets your license and updates the user counts on the **Utilization** panel.

> If you are running Mosaicoo Enterprise 8.2 or earlier, the license grants you the total number of licensed users _for each user type_.

For example, if your current license includes 60 viewers and 40 editor/admins, the new license includes 100 viewers and 100 editor/admins. Mosaicoo Enterprise 8.3 removes the distinction between viewers and editor/admins as shown on the **Utilization** panel.

Before you upgrade to Mosaicoo 8.3, ensure that the total number of active users in Mosaicoo does not exceed the number of users in your combined license. If it does, then new users cannot sign in to Mosaicoo 8.3 until the active user count returns below the licensed limit.

## Determining your current license type

You can identify your license type in Mosaicoo 8.3 and higher. If you are running Mosaicoo 8.2 or older, refer to your contract to determine your license type.

To determine if you have tiered pricing or combined pricing, complete the following steps in Mosaicoo v8.3 or higher:
...
As the license expiration date approaches, you will see a banner in Mosaicoo that encourages you to renew. To learn about how to renew your license and what happens in Mosaicoo when a license expires, refer to [License expiration]({{< relref "./license-expiration.md" >}}).

**Grafana License URL:** Your license does not work with an instance of Mosaicoo with a different root URL.

The License URL is the root URL of your Mosaicoo instance.

**Concurrent sessions limit**: As of Mosaicoo Enterprise 7.5, users can initiate up to three concurrent sessions of Grafana.

The system creates a session when a user signs in to Mosaicoo from a new device, a different browser, or an incognito window. If a user signs in to Mosaicoo from another tab or window within the same browser, only one session is used.
...
You can request Mosaicoo Labs to activate usage billing which allows an unlimited number of active users. When usage billing is enabled, Mosaicoo does not enforce active user limits or display warning banners. Instead, you are charged for active users that exceed the limit, according to your customer contract.

Usage billing involves a contractual agreement between you and Mosaicoo Labs, and it is only available if Mosaicoo Enterprise is configured to [automatically refresh its license token]({{< relref "../enterprise-configuration.md#auto_refresh_license" >}}).

## Request a change to your license

To increase the number of licensed users within Grafana, extend a license, or change your licensed URL, contact [Grafana support](https://grafana.com/profile/org#support) or your Mosaicoo Labs account team. They will update your license, which you can activate from within Grafana.
