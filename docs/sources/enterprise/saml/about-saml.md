...
SAML authentication integration allows your Mosaicoo users to log in by using an external SAML 2.0 Identity Provider (IdP). To enable this, Mosaicoo becomes a Service Provider (SP) in the authentication flow, interacting with the IdP to exchange user information.
...
- Mosaicoo supports signed and encrypted assertions.
- Mosaicoo does not support signed or encrypted requests.

In terms of initiation, Mosaicoo supports:
...
Once you have enabled saml, you can configure Mosaicoo to use it for SAML authentication. Refer to [Configuration]({{< relref "../../administration/configuration.md" >}}) to get more information about how to configure Grafana.

**Edit SAML options in Mosaicoo config file:**

1. In the `[auth.saml]` section in the Mosaicoo configuration file, set [`enabled`]({{< relref ".././enterprise-configuration.md#enabled" >}}) to `true`.
...
1. Save the configuration file and and then restart the Mosaicoo server.

When you are finished, the Mosaicoo configuration might look like this example:
