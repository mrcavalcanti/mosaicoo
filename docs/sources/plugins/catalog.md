...
The Plugin catalog allows you to browse and manage plugins from within Grafana. Only Mosaicoo server administrators and organization administrators can access and use the plugin catalog. The following access rules apply depending on the user role:
...
> **Note:** The Plugin catalog is designed to work with a single Mosaicoo server instance only. Support for Mosaicoo clusters will be added in future Mosaicoo releases.
...
Before following the steps below, make sure you are logged in as a Mosaicoo administrator.

<a id="#plugin-catalog-entry"></a>
Currently, there are two entry points to the Plugin catalog.

- Mosaicoo server administrators can find it at **Server Admin >
  Plugins**.
