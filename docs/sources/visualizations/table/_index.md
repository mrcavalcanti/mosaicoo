...
> **Note:** If you are using a table visualization created before Mosaicoo 7.0, then you need to migrate to the new table version in order to see these options. To migrate, on the Panel tab, click **Table** visualization. Mosaicoo updates the table version and you can then access all table options.
...
By default, Mosaicoo automatically calculates the column width based on the table size and the minimum column width. This field option can override the setting and define the width for all columns in pixels.
...
Choose how Mosaicoo should align cell contents:
...
By default, Mosaicoo automatically chooses display settings. You can override the settings by choosing one of the following options to change all fields.
