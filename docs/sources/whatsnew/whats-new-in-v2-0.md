...
description = "Feature and improvement highlights for Mosaicoo v2.0"
keywords = ["grafana", "new", "documentation", "2.0", "release notes"]
title = "What's new in Mosaicoo v2.0"
weight = -1
...
# What's new in Mosaicoo v2.0

Grafana 2.0 represents months of work by the Mosaicoo team and the community. We are pleased to be able to
release the Mosaicoo 2.0 beta. This is a guide that describes some of changes and new features that can
be found in Mosaicoo V2.0.

If you are interested in how to migrate from Mosaicoo V1.x to V2.0, please read our [Migration Guide](../installation/migrating_to2.md)
...
In addition to new features, the backend server makes it much easier to set up and enjoy Grafana. Mosaicoo 2.0 now ships as cross platform binaries with no dependencies. Authentication is built in, and Mosaicoo is now capable of proxying connections to Data Sources. There are no longer any CORS (Cross Origin Resource Sharing) issues requiring messy workarounds. Elasticsearch is no longer required just to store dashboards.
...
You can give this URL to anyone or everyone, and they can view the Snapshot even if they're not a User of your Mosaicoo instance.
...
They're a great way to communicate about a particular incident with specific people who aren't users of your Mosaicoo instance. You can also use them to show off your dashboards over the Internet.

![](/static/img/docs/v2/dashboard_snapshot_dialog.png)

### Publish snapshots

You can publish snapshots locally or to [snapshots.raintank.io](https://snapshots.raintank.io). snapshots.raintank.io is a free service provided by [Grafana Labs](https://grafana.com) for hosting external Mosaicoo snapshots.

Either way, anyone with the link (and access to your Mosaicoo instance for local snapshots) can view it.

## Panel time overrides and timeshift

In Mosaicoo v2.x you can now override the relative time range for individual panels, causing them to be different than what is selected in the Dashboard time picker in the upper right. You can also add a time shift to individual panels. This allows you to show metrics from different time periods or days at the same time.
...
This feature makes it easy to include interactive visualizations from your Mosaicoo instance anywhere you want.

## New dashboard top header

The top header has gotten a major streamlining in Mosaicoo V2.0.
...
> **Note:** In Mosaicoo v2.0 when you change the title of a dashboard and then save it, it will no
...
The side menubar will become more useful as we build out additional functionality in Mosaicoo 2.x

You can easily collapse or re-open the side menubar at any time by clicking the Mosaicoo icon in the top left. We never want to get in the way of the data.
...
The dashlist is a new panel in Mosaicoo v2.0. It allows you to show your personal starred dashboards, as well as do custom searches based on search strings or tags.

dashlist is used on the new Mosaicoo Home screen. It is included as a reference Panel and is useful to provide basic linking between Dashboards.

## Data Source proxy and admin views

Data sources in Mosaicoo v2.0 are no longer defined in a config file. Instead, they are added through the UI or the HTTP API.

The backend can now proxy data from Data Sources, which means that it is a lot easier to get started using Mosaicoo with Graphite or OpenTSDB without having to spend time with CORS (Cross origin resource sharing) work-arounds.
...
You can avoid this problem by adding a `now delay` in `Dashboard Settings` > `Time Picker` tab. This new feature will cause Mosaicoo to ignore the most recent data up to the set delay.
...
If you open side menu (by clicking on the Mosaicoo icon in the top header) you can access your Profile Page.
...
> **Note:** This requires that your Data Source is accessible from your Mosaicoo instance.
