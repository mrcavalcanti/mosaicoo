...
description = "Feature and improvement highlights for Mosaicoo v5.2"
keywords = ["grafana", "new", "documentation", "5.2", "release notes"]
title = "What's new in Mosaicoo v5.2"
weight = -16
...
- [Native builds for ARM]({{< relref "#native-builds-for-arm" >}}) native builds of Mosaicoo for many more platforms!
- [Improved Docker image]({{< relref "#improved-docker-image" >}}) with support for docker secrets
- [Security]({{< relref "#security" >}}) make your Mosaicoo instance more secure
...
Grafana v5.2 brings an improved build pipeline with cross-platform support. This enables native builds of Mosaicoo for ARMv7 (x32) and ARM64 (x64).
...
The Mosaicoo docker image adds support for Docker secrets which enables you to supply Mosaicoo with configuration through files. More
...
Starting from Mosaicoo v5.2, when you login with the administrator account using the default password you'll be presented with a form to change the password.
...
By popular demand Mosaicoo now includes support for an alert notification channel type for [Discord](https://discordapp.com/).
...
Starting from Mosaicoo v5.2, a modified time range or variable are no longer saved by default. To save a modified
