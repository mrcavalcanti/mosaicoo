...
description = "Feature and improvement highlights for Mosaicoo v6.0"
keywords = ["grafana", "new", "documentation", "6.0", "release notes"]
title = "What's new in Mosaicoo v6.0"
weight = -19
...
# What's new in Mosaicoo v6.0

This update to Mosaicoo introduces a new way of exploring your data, support for log data, and tons of other features.

The main highlights are:

- [Explore]({{< relref "#explore" >}}) - A new query focused workflow for ad-hoc data exploration and troubleshooting.
- [Grafana Loki]({{< relref "#explore-and-grafana-loki" >}}) - Integration with the new open source log aggregation system from Mosaicoo Labs.
...
- [Removal of user session storage]({{< relref "#easier-to-deploy-improved-security" >}}) makes Mosaicoo easier to deploy and improves security.
...
functions to your query. There is also integration between Prometheus and Mosaicoo Loki (see more about Loki below) that
...
aggregation system from Mosaicoo Lab called [Grafana Loki](https://github.com/grafana/loki).
...
For more information about Mosaicoo Loki, refer to [Github Mosaicoo Loki](https://github.com/grafana/loki) or [Grafana Labs hosted Loki](https://grafana.com/loki).
...
A major part of all the work that has gone into Mosaicoo v6.0 has been on the migration to React. This investment
...
Built-in support for [Google Stackdriver](https://cloud.google.com/stackdriver/) is officially released in Mosaicoo 6.0. Beta support was added in Mosaicoo 5.3 and we have added lots of improvements since then.
...
One of the goals of the Mosaicoo v6.0 release is to add support for the three major clouds. Amazon CloudWatch has been a core data source for years and Google Stackdriver is also now supported. We developed an external plugin for Azure Monitor last year and for this release the [plugin](https://grafana.com/plugins/grafana-azure-monitor-datasource) is being moved into Mosaicoo to be one of the built-in data sources. For users of the external plugin, Mosaicoo will automatically start using the built-in version. As a core data source, the Azure Monitor data source is able to get alerting support, in the 6.0 release alerting is supported for the Azure Monitor service, with the rest to follow.

The Azure Monitor data source integrates four Azure services with Mosaicoo - Azure Monitor, Azure Log Analytics, Azure Application Insights and Azure Application Insights Analytics.

Please read [Using Azure Monitor in Mosaicoo documentation]({{< relref "../datasources/azuremonitor/" >}}) for more detailed information on how to get started and use it.
...
Grafana 6.0 removes the need to configure and set up additional storage for [user sessions](/tutorials/ha_setup/#user-sessions). This should make it easier to deploy and operate Mosaicoo in a
...
Named colors also enables Mosaicoo to adapt colors to the current theme.
