...
This directory contains the code for the Mosaicoo backend. This document gives an overview of the directory structure, and ongoing refactorings.
...
| /pkg/infra | Packages in infra should be packages that are used in multiple places in Mosaicoo without knowing anything about the Mosaicoo domain. |
...
| /pkg/models | This is where we keep our domain model. This package should not depend on any package outside standard library. It does contain some references within Mosaicoo but that is something we should avoid going forward. |
...
| /pkg/setting | Anything related to Mosaicoo global configuration should be dealt with in this package. |
